...
load_kg_day <- stream$DISCHARGE*stream$DOC*ConvertLoadToKgDay
...
#Fill in days with no DOC load data
...
library(xlsx)
write.csv(df,file='pheasant_doc.csv',row.names=FALSE)
write.csv(df,file='yahara_doc.csv',row.names=FALSE)
write.csv(df,file='sixmile_doc.csv',row.names=FALSE)
write.csv(df,file='dorn_doc.csv',row.names=FALSE)
